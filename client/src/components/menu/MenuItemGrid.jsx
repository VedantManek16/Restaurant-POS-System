    ...
    return (
        <div className="flex-1 overflow-y-auto scrollbar-hide px-10 py-4 pb-28">
            <div className="grid grid-cols-4 gap-4 w-[100%]">
                {items?.map((item) => {
                    const quantityKey = `${selectedMenuId}-${item._id || item.id}`;
                    ...
                    return (
                        <MenuItemCard
                            ...
                            item={item}
                            quantity={quantity}
                            onIncrement={() => onIncrement(selectedMenuId, item._id || item.id)}
                            onDecrement={() => onDecrement(selectedMenuId, item._id || item.id)}
                        />
                    );
                })}
            </div>
